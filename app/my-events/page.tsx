...
function MyEventsContent() {
  ...
  const renderRow = (event: Event, editable: boolean) => {
    const expired = isExpired(event)
    return (
      <div
        key={event.id}
        className={`bg-white rounded-lg border p-4 flex items-center justify-between gap-4 ${expired ? 'opacity-75' : ''}`}
      >
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 mb-1 flex-wrap">
            ...
            {expired && (
              <span className="text-xs px-2 py-0.5 rounded-full font-medium bg-gray-700/80 text-white">Past</span>
            )}
            {event.category && (
  ...
  return (
    <main className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
      ...
      {justUpdated && (
        <div className="bg-green-50 border border-green-200 rounded-lg p-4 mb-6">
          <p className="text-green-800 font-medium">Event updated successfully.</p>
        </div>
      )}

      <div className="flex items-center justify-between mb-6 gap-3 flex-wrap">
        <div>
          <h1 className="text-2xl font-heading font-bold text-brand-dark">My Events</h1>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-white rounded-xl border">
          ...
          <Link
            href="/events/create"
            className="bg-brand-gold hover:bg-brand-gold/90 text-brand-dark px-6 py-2.5 rounded-lg font-semibold text-sm transition-colors"
          >
