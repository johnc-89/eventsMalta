...
- 0030 — `event-images` storage bucket hardening: owner-scoped INSERT/UPDATE/DELETE policies (was any-authenticated) + bucket `file_size_limit` 5 MB and `allowed_mime_types` (jpeg/png/webp).
- 0031 — DB-side event cap: `enforce_event_cap()` + BEFORE INSERT trigger on `events` rejecting non-staff inserts past `max_active_events` (admin/super_admin exempt; NULL = unlimited; importer aggregator bumped to 1M).
- 0032 — Data cleanup: null out malformed `events.ticket_url` (not matching `^https?://`) that Google flagged as invalid `offers.url` in the Event JSON-LD. Paired with app-side `sanitizeHttpUrl()` ([lib/url.ts](lib/url.ts)) at the render + both write paths.
- 0033 — Fix the super-admin delete button ([components/SuperAdminDeleteButton.tsx](components/SuperAdminDeleteButton.tsx)). Its `super_admin_delete_event(bigint)` RPC had only ever lived in the dashboard (never in a migration — 0011 just mentions it). Authoritatively (re)defines it: `SECURITY DEFINER`, super_admin guard, explicit child-row cleanup (`saved_events`/`event_images`/`event_occurrences`) so the hard delete can't trip an FK, `RETURNS boolean` (the button's `if (!data)` needs a truthy value on success), `EXECUTE` to `authenticated` only.
- 0035 — Data-only: merge 4 overlapping category pairs (`Family / Kids`→`Family Friendly`, `Rooftop`→`Outdoor`, `Arts`→`Culture & Arts`, `Nightlife`→`Party`) — rewrite `events.tags[]`, backfill winner icon, drop the 4 losing `tags` rows. Applied via one-off script, not the SQL editor (pure DML). Paired with 301 redirects in `next.config.js` for the retired `/events/tag/<slug>` landing pages.
- 0034 — **Verified-user event claiming.** Adds `profiles.is_verified` (admin-granted; self-verify blocked by extending the 0022 `enforce_profile_role_change` trigger) and `events.claimed_by`/`claimed_at` (FK `events_claimed_by_fkey`). RPCs (SECURITY DEFINER): `claim_event(bigint)` — a verified, non-suspended user claims an approved, unclaimed event; `unclaim_event(bigint)` — the claimant or any admin releases it; `get_public_organiser(uuid)` — safe public fields (`display_name/avatar_url/bio/is_verified`) for a **verified, non-deleted, non-suspended** profile only, powering `/organisers/<id>` without widening the 0021 anon column grant. Claiming is attribution-only (does not change `organizer_id`/edit rights/event cap). UI: claim button + "Claimed by …" marker on the event page ([components/ClaimEventButton.tsx](components/ClaimEventButton.tsx)), public organiser page ([app/organisers/[id]/page.tsx](app/organisers/[id]/page.tsx)), Verify toggle in [/admin/users](app/admin/users/page.tsx). **Apply 0034 before deploying** — the event-detail `claimant` embed 404s every event page until the FK exists. Also patch the dashboard-only `admin_list_profiles` RPC to return `is_verified`.
...
- 0036 — Two fixes to `/admin/duplicates` ([app/admin/duplicates/page.tsx](app/admin/duplicates/page.tsx)): (1) adds `event_duplicate_dismissals` (admin/super_admin RLS) so a "Not duplicates — stop flagging" button per group can mark specific event-id pairs as reviewed; the union-find matcher skips any pair present in the table. (2) redefines `super_admin_delete_event` (0033) from a hard `DELETE FROM events` to a soft-delete (`deleted_at = now()`). Reasoning: deleting a flagged duplicate via the public event page's super-admin button erased `deleted_at` along with the row, so `lib/importers/pipeline.ts`'s existing dedup check (skip re-import when `existing.deleted_at` is set) had nothing to find, and the next adapter run reinserted the "deleted" event. The button is now labelled "Hide event" to match. Also excludes past events (`date_start < now`) from the duplicate-matching pool in the same page, since they're no longer actionable.
